...
# Date              : Sun 19 Sep 2021 22:08:55
# Last Modified Date: Thu 14 Oct 2021 18:35:29
...
def run_training(args: TrainArgs,
	...
	elif not args.transfer_learning:
	...
	if args.transfer_learning:
		target_data = get_data(path=args.target_data_path, args=args, logger=logger)
		target_train_data, target_val_data, target_test_data = split_data(data=target_data, split_type=args.split_type, sizes=args.split_sizes, seed=args.seed, num_folds=args.num_folds, args=args, logger=logger)

		_data = [_ for _ in data]
		for each in target_train_data:
				_data.append(each)
		train_data = MoleculeDataset(_data)
		val_data, test_data = target_val_data, target_test_data
	...
	for model_idx in range(args.ensemble_size):
		...
		for epoch in trange(args.epochs):
			...
			if args.train_eval:
				...
				train_eval_loader = MoleculeDataLoader(dataset=data,
									batch_size=len(data),
									num_workers=num_workers)
				...
				# evaluate the mdoel only on train split of target data
				train_eval_loader = MoleculeDataLoader(dataset=target_train_data,
									batch_size=len(target_train_data),
									num_workers=num_workers)
