...
from aada.features import get_features_generator
from aada.features import BatchMolGraph, MolGraph
...
def cache_graph() -> bool:
    r"""Returns whether :class:`~aada.features.MolGraph`\ s will be cached."""
    return CACHE_GRAPH


def set_cache_graph(cache_graph: bool) -> None:
    r"""Sets whether :class:`~aada.features.MolGraph`\ s will be cached."""
    global CACHE_GRAPH
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        r"""
        Constructs a :class:`~aada.features.BatchMolGraph` with the graph featurization of all the molecules.

        .. note::
           The :class:`~aada.features.BatchMolGraph` is cached in after the first time it is computed
           and is simply accessed upon subsequent calls to :meth:`batch_graph`. This means that if the underlying
           set of :class:`MoleculeDatapoint`\ s changes, then the returned :class:`~aada.features.BatchMolGraph`
           will be incorrect for the underlying data.

        :return: A list of :class:`~aada.features.BatchMolGraph` containing the graph featurization of all the
    ...
    def normalize_features(self, scaler: StandardScaler = None, replace_nan_token: int = 0) -> StandardScaler:
        """
        Normalizes the features of the dataset using a :class:`~aada.data.StandardScaler`.

        The :class:`~aada.data.StandardScaler` subtracts the mean and divides by the standard deviation
        for each feature independently.

        If a :class:`~aada.data.StandardScaler` is provided, it is used to perform the normalization.
        Otherwise, a :class:`~aada.data.StandardScaler` is first fit to the features in this dataset
        and is then used to perform the normalization.

        :param scaler: A fitted :class:`~aada.data.StandardScaler`. If it is provided it is used,
                       otherwise a new :class:`~aada.data.StandardScaler` is first fitted to this
                       data and is then used.
        :param replace_nan_token: A token to use to replace NaN entries in the features.
        :return: A fitted :class:`~aada.data.StandardScaler`. If a :class:`~aada.data.StandardScaler`
                 is provided as a parameter, this is the same :class:`~aada.data.StandardScaler`. Otherwise,
                 this is a new :class:`~aada.data.StandardScaler` that has been fit on this dataset.
        """
        if len(self._data) == 0 or self._data[0].features is None:
            return None
    ...
    def normalize_targets(self) -> StandardScaler:
        """
        Normalizes the targets of the dataset using a :class:`~aada.data.StandardScaler`.
        ...
        :return: A :class:`~aada.data.StandardScaler` fitted to the targets.
        """
...
def construct_molecule_batch(data: List[MoleculeDatapoint]) -> MoleculeDataset:
    ...
    Additionally, precomputes the :class:`~aada.features.BatchMolGraph` for the constructed
