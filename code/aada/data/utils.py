...
from aada.features import load_features, load_valid_atom_features
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
	...
	:param data: A :class:`~aada.data.MoleculeDataset`.
	:return: A :class:`~aada.data.MoleculeDataset` with only the valid molecules.
	"""
...
def filter_common_smiles(source_data: MoleculeDataset, target_data: MoleculeDataset)\
		-> Tuple[MoleculeDataset, MoleculeDataset]:
	"""
	Filters out invalid SMILES.

	:param source_data: A :class:`~aada.data.MoleculeDataset`.
	:param target_data: A :class:`~aada.data.MoleculeDataset`.
	:return: A Tuple of :class:`~aada.data.MoleculeDataset` with only the molecules that are not common.
	"""
	temp_source, temp_target = [], []

	source_smiles = [_.smiles[0] for _ in source_data]
	target_smiles = [_.smiles[0] for _ in target_data]

	common_smiles = set(source_smiles) & set(target_smiles)

	return MoleculeDataset([sd for sd in source_data if sd.smiles[0] not in common_smiles]),\
		MoleculeDataset([td for td in target_data if td.smiles[0] not in common_smiles])
...
def get_data(path: str,
	...
	:param args: Arguments, either :class:`~aada.args.TrainArgs` or :class:`~aada.args.PredictArgs`.
	...
	:param store_row: Whether to store the raw CSV row in each :class:`~aada.data.data.MoleculeDatapoint`.
	...
	:return: A :class:`~aada.data.MoleculeDataset` containing SMILES and target values along
...
def get_data_from_smiles(smiles: List[List[str]],
	...
	"""
	Converts a list of SMILES to a :class:`~aada.data.MoleculeDataset`.
	...
	:return: A :class:`~aada.data.MoleculeDataset` with all of the provided SMILES.
	"""
	debug = logger.debug if logger is not None else print
...
def split_data(data: MoleculeDataset,
	...
	:param args: A :class:`~aada.args.TrainArgs` object.
	:param logger: A logger for recording output.
	:return: A tuple of :class:`~aada.data.MoleculeDataset`\ s containing the train,
...
def get_class_sizes(data: MoleculeDataset) -> List[List[float]]:
	...
	:param data: A classification :class:`~aada.data.MoleculeDataset`.
