...
from aada.features import BatchMolGraph, get_atom_fdim, get_bond_fdim, mol2graph
from aada.utils.nn_utils import index_select_ND, get_activation_function
...
class MPNEncoder(nn.Module):
	...
	def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int):
		...
		if self.aggregation == 'self-attention' or self.aggregation == 'hier-attention' or self.aggregation == 'all-concat':
			self.W_att_1 = nn.Linear(self.hidden_size, args.attn_dim)
			self.W_att_2 = nn.Linear(args.attn_dim, 1)
			self.save_attention_scores = False # args.save_attention_scores
			self.save_attention_path = os.path.join(args.save_dir, "attn_scores.txt")
	...
	def forward(self,
		...
		:param mol_graph: A :class:`~aada.features.featurization.BatchMolGraph` representing
	...
	#		atom_hiddens = self.dropout_layer(atom_hiddens)								# num_atoms x (hidden + descriptor size)
		...
		for i, (a_start, a_size) in enumerate(a_scope):
			if a_size == 0:
				...
				# addition of A-DMPNN
				elif self.aggregation == 'self-attention':
					temp_h = self.act_func(self.W_att_1(mol_vec))
					scores = torch.softmax(self.W_att_2(temp_h), dim=0)
					x = (1 + scores) * mol_vec
					mol_vec = x.sum(dim=0)

					# optionally store attention scores
					if self.save_attention_scores:
						save_scores(scores, self.save_attention_path)

				mol_vecs.append(mol_vec)

		mol_vecs_new = []
		if self.aggregation == 'identity':
			zero_pad = torch.Tensor([0]*100)
			max_atoms = max([_.shape[0] for _ in mol_vecs])
			for mol_vec in mol_vecs:
				if max_atoms - mol_vec.shape[0]:
					mol_vecs_new.append(torch.cat((mol_vec, torch.stack([zero_pad \
							for _ in range(max_atoms - mol_vec.shape[0])])), dim=0))
				else:
					mol_vecs_new.append(mol_vec)

			mol_vecs = mol_vecs_new
...
class MPN(nn.Module):
	...
	def forward(self,
		...
		:param batch: A list of list of SMILES, a list of list of RDKit molecules, or a
					  :class:`~aada.features.featurization.BatchMolGraph`.
