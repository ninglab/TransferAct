...
# Date              : Sat 16 Oct 2021 16:54:42
# Last Modified Date: Sat 16 Oct 2021 16:54:42
...
from aada.data import set_cache_mol
from aada.features import get_available_features_generators
...
class CommonArgs(Tap):
    ...
    """Whether to save atom-wise  attention scores of every molecule"""
    no_cuda: bool = False
    ...
    def features_scaling(self) -> bool:
        """Whether to apply normalization with a :class:`~aada.data.scaler.StandardScaler` to the additional molecule-level features."""
...
class TrainArgs(CommonArgs):
    ...
    # Model arguments
    model: str = "aada"
    """Which model to use: AADA/DANN/CADA to train"""
    bias: bool = False
    ...
    attn_dim: int = 100
    """Dimensionality of attention layers in MPN."""
    ...
    local_discriminator_hidden_size: int = 100
    """Hidden dim for local discriminator (defaults to 100)."""
    local_discriminator_num_layers: int = 2
    """Number of layers in FFN of local discriminator."""
    global_discriminator_hidden_size: int = 100
    """Hidden dim for local discriminator (defaults to 100)."""
    global_discriminator_num_layers: int = 2
    """Number of layers in FFN of local discriminator."""
    ffn_hidden_size: int = 100
    ...
    aggregation: Literal['mean', 'sum', 'norm', 'identity', 'self-attention', 'hier-attention', 'all-concat'] = 'sum'
    ...
    """Trade-off parameter for source and target classification error."""
    alpha: float = 1.0
    """Trade-off parameter for discriminator and classification error."""
    lamda: float = 0.5
    """Ablation studies: use `exclude_local` to exclude local_disc component, use `exclude_global` to exclude global_disc component
    use both to add both to the loss fn"""
    ...
    def process_args(self) -> None:
        ...
        for metric in self.metrics:
            if not ((self.dataset_type == 'classification' and metric in ['auc', 'prc-auc', 'accuracy', 'binary_cross_entropy' \
									'sensitivity', 'precision', 'recall', 'specificity', 'f1_score']) or
