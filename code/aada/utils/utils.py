...
from sklearn.metrics import auc, mean_absolute_error, mean_squared_error, precision_recall_curve, r2_score,\
	roc_auc_score, accuracy_score, log_loss, precision_score, recall_score, f1_score, confusion_matrix
import torch
...
from aada.data import StandardScaler, MoleculeDataset
from aada.modules import MoleculeModel, AADALoss, DANNLoss
...
def save_checkpoint(path: str,
	...
	:param scaler: A :class:`~aada.data.scaler.StandardScaler` fitted on the data.
	:param features_scaler: A :class:`~aada.data.scaler.StandardScaler` fitted on the features.
	:param args: The :class:`~aada.args.TrainArgs` object containing the arguments the model was trained with.
...
def load_checkpoint(path: str,
	...
	:return: The loaded :class:`~aada.models.model.MoleculeModel`.
	"""
...
def load_scalers(path: str) -> Tuple[StandardScaler, StandardScaler]:
	...
	:return: A tuple with the data :class:`~aada.data.scaler.StandardScaler`
			 and features :class:`~aada.data.scaler.StandardScaler`.
	"""
	state = torch.load(path, map_location=lambda storage, loc: storage)
...
def load_args(path: str) -> TrainArgs:
	...
	:param path: Path where model checkpoint is saved.
	:return: The :class:`~aada.args.TrainArgs` object that the model was trained with.
	"""
	args = TrainArgs()
...
def precision(targets: List[int], preds: List[float]) -> float:
	return precision_score(targets, np.asarray(preds)>0.5)

def recall(targets: List[int], preds: List[float]) -> float:
	return recall_score(targets, np.asarray(preds)>0.5)

def _f1_score(targets: List[int], preds: List[float]) -> float:
	return f1_score(targets, np.asarray(preds)>0.5)

def specificity(targets: List[int], preds: List[float]) -> float:
	tn, fp, fn, tp = confusion_matrix(targets, np.asarray(preds)>0.5).ravel()
	return tn/(tn+fp)
...
def build_optimizer(model: nn.Module, args: TrainArgs) -> Optimizer:
	...
	:param args: A :class:`~aada.args.TrainArgs` object containing optimizer arguments.
	...
	if not args.exclude_local:
		params.append({'params': model.discriminator_local.parameters(), 'lr': args.init_lr, 'weight_decay': 0})
	if not args.exclude_global:
		params.append({'params': model.discriminator_global.parameters(), 'lr': args.init_lr, 'weight_decay': 0})
	"""
	return Adam(params)
...
def build_lr_scheduler(optimizer: Optimizer, args: TrainArgs, total_epochs: List[int] = None) -> _LRScheduler:
	...
	:param args: A :class:`~aada.args.TrainArgs` object containing learning rate arguments.
...
def save_smiles_splits(data_path: str,
	...
	:param train_data: Train :class:`~aada.data.data.MoleculeDataset`.
	:param val_data: Validation :class:`~aada.data.data.MoleculeDataset`.
	:param test_data: Test :class:`~aada.data.data.MoleculeDataset`.
